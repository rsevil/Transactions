...
namespace Utils.Transactions.IO.Files.Local.Tests
{
    public class LocalFileSystemManagerTest : IDisposable
    {
        private int _numTempFiles;
        private LocalFileSystemManager _target;

        public LocalFileSystemManagerTest()
        {
            _target = new LocalFileSystemManager();
        ...
        public void Dispose()
        {
            int numTempFiles = Directory.GetFiles(FilesHelper.GetTempFolderPath()).Length;
        ...
        public void CanWriteAllTextAndRenameAndRollback()
        {
            ...
            try
            {
                ...
                using (TransactionScope scope1 = new TransactionScope())
                {
                    f2 = _target.WriteAllText(f1, contents2, renameIfExists: true);
                }
